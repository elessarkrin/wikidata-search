                        ...
                        for (var j in entity.claims) {
                            ...
                            for (var k in claim) {
                                ...
                                if (snak.hasOwnProperty('datavalue') && snak.datavalue.hasOwnProperty('value')) {
                                    ...
                                    else if (valueType === 'string' || valueType === 'url' || valueType === 'external-id')
                                        val = snak.datavalue.value;
                                    else if (valueType === 'time') {
                                        val = snak.datavalue.time;

                                        if (!val && snak.datavalue.value)
                                            val = moment(snak.datavalue.value.time.substr(1, snak.datavalue.value.time.length)).utcOffset(snak.datavalue.value.timezone || 0).toDate().getTime();

                                    } else if (valueType === 'globe-coordinate' && snak.datavalue.hasOwnProperty('longitude') && snak.datavalue.hasOwnProperty('latitude'))
                                        val = snak.datavalue.longitude.toString() + "," + snak.datavalue.latitude.toString()  //long,lat order
                                    else if (valueType === 'quantity' && snak.datavalue.value.hasOwnProperty('amount')) {
                                        val = parseInt(snak.datavalue.value.amount);
                                        if (snak.datavalue.value.unit != '1') { //if we have a unit, suffix it to the string.

                                            var unitId = snak.datavalue.value.unit.match(/Q.+$/)[0];

                                            if(!unitMaps[unitId]) {
                                                var combinedUri = 'https://' + wikidataSearch.options.apiHost + wikidataSearch.options.apiPath;
                                                combinedUri = combinedUri + '?action=' + wikidataSearch.options.getAction;
                                                combinedUri = combinedUri + '&languages=' + wikidataSearch.options.language;
                                                combinedUri = combinedUri + '&props=aliases&normalize=true';
                                                combinedUri = combinedUri + '&ids=' + unitId;
                                                combinedUri = combinedUri + '&format=json'; //we always want JSON back...

                                                var unitObj = JSON.parse(syncRequest('GET', combinedUri).body);

                                                val = val.toLocaleString(wikidataSearch.options.locale) + ' ' + unitObj.entities[unitId].aliases[wikidataSearch.options.language][0].value;

                                                //Add to unit map
                                                unitMaps[unitId] =  unitObj.entities[unitId].aliases[wikidataSearch.options.language][0].value;
                                            }else {
                                                val = val.toLocaleString(wikidataSearch.options.locale) + ' ' + unitMaps[unitId];
                                            }
                                        }
                                    } else if (valueType === 'commonsMedia') {

                                        var combinedUri = 'https://commons.wikimedia.org/w/api.php';
                                        combinedUri = combinedUri + '?action=query';
                                        combinedUri = combinedUri + '&format=json'; //we always want JSON back...
                                        combinedUri = combinedUri + '&prop=imageinfo&iiprop=url&titles=File:' + snak.datavalue.value.replace(/\s/g, '_');


                                        var imageObj = JSON.parse(syncRequest('GET', combinedUri).body);

                                        val = imageObj.query.pages;

                                        try {
                                            for (var key in imageObj.query.pages) {
                                                if (key != '-1')
                                                    val = imageObj.query.pages[key].imageinfo[0].url
                                            }
                                        } catch (e) {
                                            //TODO:do something
                                        }
                            ...
                            if (prop && val && propType) {
                                ...
                                if (!resolveProperties)
                                    ...
                                    claims.push({
                                        'property': prop,
                                        'value': moment(val).utcOffset(0).format('Do MMMM YYYY'),
                                        'time': val,
                                        'type': propType,
                                        'propertyCached': propertyCached,
