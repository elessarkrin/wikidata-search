                        ...
                        for (var j in entity.claims) {
                                ...
                                if (snak.hasOwnProperty('datavalue') && snak.datavalue.hasOwnProperty('value')) {
                                    ...
                                    if (valueType === 'wikibase-item')
                                        ...
                                        if(!val && snak.datavalue.value)
                                            val = moment(snak.datavalue.value.time).utcOffset(snak.datavalue.value.timezone || 0).toDate().getTime();
                            ...
                            if (prop && val && propType) {
                                ...
                                if (!resolveProperties)
                                    ...
                                    claims.push({'property': prop, 'value': moment(val.substr(1, val.length)).utcOffset(0).format('Do MMMM YYYY'), 'time': val, 'type': propType, 'propertyCached' : propertyCached, 'valueCached' : valueCached });
